<template>
     <nav class="tableBar"
          ...
          <ul class="navList displayFlex flexJustifyAround">
               <li :class="(activenum === index) ?'active':'link'"
                    v-for="(val,index) in list"
                    v-anchor="val.num"
                    :key="index">
                    <span style="position: relative"
                         :class="val.name == '全景'?upDateIcom:''">{{val.name}}</span>
               </li>
          </ul>
     </nav>
</template>
<script>
import anchor from '../../common/js/href.js'
export default {
     props: {
          nums: 0,
          isUpdate: {
     ...
     methods: {
          ...
          initBar(obj) {
               if (!this.obj.scoreData || this.obj.scoreData.stars == 0) {
               ...
               // if (this.obj.constructionSite.constructionList.length == 0) {
               //      let spliceNum = this.list.findIndex((val, index, arr) => {
               //           return val.num == 3;
               //      })
               //      this.list.splice(spliceNum, 1);
