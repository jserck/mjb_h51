    ...
    <div class="consultDate" v-if="isshow">
        <div class="consultDateTop">
            <div class="consultDateText">
                <h1>{{consultDate.maijiab.text}}</h1>
                <p>{{consultDate.author}}</p>
                <div class="consultDateText_date displayFlex flexJustifybetween">
                    <span>{{consultDate.maijiab.right}}</span>
                    <span>阅读{{consultDate.readNum || 0}}次</span>
                </div>
                <Icon :data="consultDate.maijiab.labels"/>
                <!-- <div class="consultDateText_type ">
        ...
        <div class="g-mzsm" v-if="consultDate.statement">
            <p>{{consultDate.statement}}</p>
        </div>
        <div class="recommend_" v-if="consultDate.statics.length!==0">
            <p class="recommend_title">其他推荐</p>
            <div
                class="inner_"
                @click="click_handler(value)"
                v-for="(value,key) in consultDate.statics"
                :key="key"
            >
                ...
                <div class="right_inner" :style="`background:url(${value.img ? value.img : $url.error_img});background-size:cover`">
                    <!-- <img :src="value.img ? value.img : $url.error_img" alt> -->
                </div>
            </div>
        </div>
...
export default {
    data() {
        return {
            ...
            href_app_is: false,
            downLoadUrl:
                "https://test.homehawkeye.com/appDown.html?ut=1&ui=466",
            consultDate: {
                maijiab: {
                    labels: [],
                    left: "",
    ...
    methods: {
        ...
        click_handler(value) {
            ...
            this.consult_obj.url =
                this.$url.consult_url + id.substring(1, id.length) || "";
            this.consult_obj.openUrl = this.$url.consult_url + ids || "";
            ...
            var p = Ob.is_Weixin()
                ...
                : Ob.is_WeiBo()
                    ...
                    : Ob.isQq()
                        ? "1"
                        : "0";
            this.downLoadUrl = `https://test.homehawkeye.com/appDown.html?ut=1&ui=466&p=${p}`;
